...
//   const [selectedSetNumber, setSelectedSetNumber] = useState(""); // new state
...
//         if (answer.isCorrect) acc[answer.domain].correct++;
//         return acc;
//       }, {})
//     }
//   };
...
//   // 🔥 Remove the current set after download
//   if (currentSet) {
//     setAvailableSets(prev => prev.filter(set => set.setId !== currentSet.setId));
//     setUsedSets(prev => prev.filter(id => id !== currentSet.setId));
//     setCurrentSet(null); 
//   }
// };
...
//           <input
//             type="text"
//             placeholder="Enter Team's Flag Colour"
//             value={teamName}
...
//             Enter Flag Color Found (name, e.g., red, blue, orange)
//           </label>
//           <input
//             type="text"
//             list="color-options"
...
//             onChange={(e) => setSelectedSetNumber(e.target.value)}
//             className="w-full p-3 rounded-lg bg-gray-700 border border-gray-600 focus:border-blue-400 focus:outline-none mb-4 text-white"
//           />

//           <button
//   onClick={() => {
//     const set = questionSets.find(s => s.setId === parseInt(selectedSetNumber));
//     if (set) {
//       setCurrentSet(set);
//       setAvailableSets([]); // optional: disables "Change Set"
//       setUsedSets([set.setId]);
//       setCurrentQIndex(0);
//       setScore(0);
//       setSelectedOption("");
//       setFeedback("");
...
//       setStartTimestamp(new Date().toISOString());
//       setEndTimestamp(null);
//       setAllSetsInvolved([{ ...set }]);
//       setSetChangeLog([]);
//       setStep("quiz");
//     } else {
//       alert("Invalid Set Number!");
//     }
//   }}
//   disabled={!teamName || !flagColor || !selectedSetNumber}
...
export default function App() {
  ...
  const [finished, setFinished] = useState(false);
  const [userAnswers, setUserAnswers] = useState([]); // Store user answers
  const [selectedSetNumber, setSelectedSetNumber] = useState(""); // Set number input
    ...
    <div className="min-h-screen bg-gray-900 text-white flex items-center justify-center p-6">
      ...
      {step === "start" && (
        <div className="bg-gray-800 p-8 rounded-2xl shadow-xl max-w-md w-full text-center border border-gray-700">
          ...
          <input
            type="text"
            placeholder="Enter Coordinator's Flag Colour"
            value={teamName}
            onChange={(e) => setTeamName(e.target.value)}
            className="w-full p-3 rounded-lg bg-gray-700 border border-gray-600 focus:border-blue-400 focus:outline-none mb-4 text-white"
          />
      ...
      {step === "quiz" && currentSet && (
        <div className="bg-gray-800 p-8 rounded-2xl shadow-xl max-w-2xl w-full border border-gray-700">
          ...
          {/* Set Information - Display the input set number */}
          <div className="bg-gray-700 p-3 rounded-lg mb-4 border border-gray-600">
            <div>
              <span className="text-blue-400 font-semibold">Set {selectedSetNumber}</span>
              <span className="text-gray-400 ml-2">({currentSet.questions[currentQIndex].domain})</span>
            </div>
          </div>
      ...
      {step === "result" && (
        <div className="bg-gray-800 p-8 rounded-2xl shadow-xl max-w-2xl w-full border border-gray-700">
          ...
          <div className="text-center mb-6">
            <p className="mb-2 text-lg">Team: <span className="font-semibold text-blue-400">{teamName}</span></p>
            <p className="mb-2 text-sm text-gray-400">Question Set: <span className="font-semibold text-yellow-400">Set {selectedSetNumber}</span></p>
            <p className="mb-4 text-xl">Final Score: <span className="font-bold text-blue-400">{score} / {currentSet?.questions.length || 5}</span></p>
